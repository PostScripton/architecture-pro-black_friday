...
Схема: 1 config server + 2 шарда, каждый шард - реплика сет из 3 узлов + mongos router + API приложение.

Отличие от `mongo-sharding`: каждый шард теперь не одиночный узел, а реплика сет из 3 узлов (1 primary + 2 secondary). Это обеспечивает отказоустойчивость: при падении primary один из secondary автоматически становится новым primary.
...
Эндпоинты:
- `GET /` - топология MongoDB, список шардов, реплика сеты и количество документов в коллекциях
